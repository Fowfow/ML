...
def default_kwargs_base():
    ...
    return dict(y=y, n=y.shape[0], np=nperiod, ns=35, nt=19, nl=13, no=2, ni=1,
                nsjump=4, ntjump=2, nljump=2, isdeg=1, itdeg=1, ildeg=1, rw=rw,
                trend=trend, season=season, work=work)
...
def default_kwargs_short():
    ...
    trend = np.zeros(nobs)
    season = np.zeros(nobs)
    kwargs.update(dict(y=y, n=nobs, rw=rw, trend=trend,
...
def _to_class_kwargs(kwargs, robust=False):
    ...
    class_kwargs = dict(endog=endog, period=np, seasonal=ns, trend=nt,
                        low_pass=nl,
                        seasonal_deg=isdeg, trend_deg=itdeg,
                        low_pass_deg=ildeg,
                        robust=robust, seasonal_jump=nsjump, trend_jump=ntjump,
