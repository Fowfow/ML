...
class TestDistDependenceMeasures(object):
    ...
    def test_results_on_the_iris_dataset(self):
        ...
        """
        try:
            iris = get_rdataset("iris",cache=True)
        except:
            pytest.skip("no internet and not cached (expected in autopkgtests and arch:any, should pass on arch:all)")
        iris=iris.data.values[:, :4]
        x = iris[:50]
        y = iris[50:100]
    ...
    def test_results_on_the_quakes_dataset(self):
        ...
        """
        try:
            quakes = get_rdataset("quakes",cache=True)
        except:
            pytest.skip("no internet and not cached (expected in autopkgtests and arch:any, should pass on arch:all)")
        quakes=quakes.data.values[:, :3]
        x = quakes[:50]
        y = quakes[50:100]
