# Help on how to use this library through examples :
...
def pps_matrix(dtf, annotation=True, lst_filters=[], figsize=(10,5), filename="pps.png"):
    ...
    if (len(lst_filters) != 0):
        dtf_pps = dtf_pps.loc[lst_filters]
    fig, ax = plt.subplots(figsize=figsize)
...
def data_preprocessing(dtf, y, processNas=None, processCategorical=[], split=None, scale=None, task="classification"):
    '''
    ...
    :parameter
        ...
        :param processCategorical: list of categorical features to process
        :param split: num or None - test_size (example 0.2)
        :param scale: str or None - "standard", "minmax"
        :param task: str - "classification" or "regression" (only if scale is not None)
    :return
    ...
    try:
        ...
        for col in dtf.drop(y, axis=1).columns.to_list():
            if dtf[col].dtype == "O" or utils_recognize_type(dtf, col, max_cat=20) == "cat":
                print("\t", "WARNING:", col, "-->", dtf[col].nunique(), "categories")
                cols_with_categorical.append(col)
        ### treat
        if len(cols_with_categorical) != 0 and len(processCategorical) > 0:
            print("\t", "...treating categorical...")
            for col in cols_with_categorical:
                if (col in processCategorical):
                    print("\t", col)
                    dtf = pd.concat([dtf, pd.get_dummies(
                        dtf[col], prefix=col, drop_first=True)], axis=1).drop([col], axis=1)
        else:
...
def features_selection(dtf, y, top=10, task="classification", figsize=(20,10), filename="features_selection.png"):
    ...
    try:
        ...
        dtf_features["anova(p_value)"] = dtf_features["features"].apply(lambda x: "anova(p_value)" if x in pvalue_selected_features else "")
        dtf_features["num1"] = dtf_features["features"].apply(lambda x: 1 if x in pvalue_selected_features else 0)
        dtf_features["lasso(regularization)"] = dtf_features["features"].apply(lambda x: "lasso(regularization)" if x in regularization_selected_features else "")
        dtf_features["num2"] = dtf_features["features"].apply(lambda x: 1 if x in regularization_selected_features else 0)
        dtf_features["method"] = dtf_features[["anova(p_value)","lasso(regularization)"]].apply(lambda x: (x[0]+" "+x[1]).strip(), axis=1)
...
def utils_threshold_selection(model, X, y, figsize=(10,5), filename="utils_threshold_selection.png"):
    ...
    for k in dic_scores.keys():
        print("\t", k, "--> best threshold:", round(dtf_scores[dtf_scores[k]==dtf_scores[k].max()][k].index[0], 2))
...
def tune_classif_model(X_train, y_train, model_base=None, param_dic=None, scoring="f1", searchtype="RandomSearch", n_iter=1000, cv=10, figsize=(10,5), filename1="tune_classif_model1.png", filename2="tune_classif_model2.png"):
    ...
    if searchtype == "RandomSearch":
        random_search = model_selection.RandomizedSearchCV(model_base, param_distributions=param_dic, n_iter=n_iter, scoring=dic_scores, refit=scoring, n_jobs=20).fit(X_train, y_train)
        print("Best Model parameters:", random_search.best_params_)
        print("Best Model "+scoring+":", round(random_search.best_score_, 2))
        model = random_search.best_estimator_
        
    elif searchtype == "GridSearch":
        grid_search = model_selection.GridSearchCV(model_base, param_dic, scoring=dic_scores, refit=scoring, n_jobs=20).fit(X_train, y_train)
    ...
    Kfold_base = model_selection.cross_validate(estimator=model_base, X=X_train, y=y_train, cv=cv, scoring=dic_scores, n_jobs=20)
...
def evaluate_classif_model(y_test, predicted, predicted_prob, show_thresholds=True, figsize=(25,5), filename="evaluate_classif_model.png"):
    ...
    '''
    print("\t", "--- Evaluate chosen classification model ---")
...
def tune_regr_model(X_train, y_train, model_base=None, param_dic=None, scoring="r2", searchtype="RandomSearch", n_iter=1000, cv=10, figsize=(10,5), filename="tune_regr_model.png"):
    ...
    print("---", searchtype, "---")
    if searchtype == "RandomSearch":
        random_search = model_selection.RandomizedSearchCV(model_base, param_distributions=param_dic, n_iter=n_iter, scoring=scoring, n_jobs=20).fit(X_train, y_train)
        print("Best Model parameters:", random_search.best_params_)
        print("Best Model "+scoring+":", round(random_search.best_score_, 2))
        model = random_search.best_estimator_
    
    elif searchtype == "GridSearch":
        grid_search = model_selection.GridSearchCV(model_base, param_dic, scoring=scoring, n_jobs=20).fit(X_train, y_train)
        print("Best Model parameters:", grid_search.best_params_)
        print("Best Model mean "+scoring+":", round(grid_search.best_score_, 2))
        model = grid_search.best_estimator_
...
def utils_dimensionality_reduction(X_train, X_test, n_features=2):
    '''
    Decomposes the feature matrix of train and test.
    :parameter
        :param X_train: array
        :param X_test: array
